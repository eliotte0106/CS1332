...
public class ExternalChainingHashMap<K, V> {
    ...
    public void resizeBackingTable(int length) {
        if (length < size) {
            throw new IllegalArgumentException("length cannot be less than the number of items");
        }
        ExternalChainingMapEntry<K, V>[] copyTable = new ExternalChainingMapEntry[length];
        for (ExternalChainingMapEntry entry : table) {
            if (entry != null) {
                int idx = Math.abs(entry.getKey().hashCode() % copyTable.length);
                copyTable[idx] = entry;
            }
        }
        table = copyTable;
